...
- `BIBLIOTALK_API_KEY` (可选；若提供，会写回 `gurus/.env`)

运行时会优先使用 `gurus/.env` 中的：
- `API_BASE_URL`
- `BIBLIOTALK_API_KEY`
"""
...
1. 从仓库根目录的 `gurus/.env` 读取 `API_BASE_URL` 与 `BIBLIOTALK_API_KEY`。
2. 调用 Bibliotalk 时使用请求头 `x-api-key: $BIBLIOTALK_API_KEY`。
...
def _guru_env_path(base_dir: Path) -> Path:
    return base_dir / DEFAULT_GURU_ENV_FILE
...
def _write_env_file(path: Path, env_data: dict[str, str]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    ordered_keys = ["API_BASE_URL", "BIBLIOTALK_API_KEY"]
    remaining_keys = sorted(k for k in env_data.keys() if k not in ordered_keys)

    lines = [
        "# Generated by scripts/skill_writer.py for local guru skills.",
...
def _apply_guru_env(base_dir: Path) -> dict[str, str]:
    env_data = _read_env_file(_guru_env_path(base_dir))
    for key in ("API_BASE_URL", "BIBLIOTALK_API_KEY"):
        value = env_data.get(key, "").strip()
        if value:
...
def ensure_guru_api_env(base_dir: Path) -> dict[str, str]:
    env_path = _guru_env_path(base_dir)
    env_data = _read_env_file(env_path)

    api_url = (
        env_data.get("API_BASE_URL")
        or os.environ.get("API_BASE_URL")
        or DEFAULT_API_BASE_URL
    ).rstrip("/")

    api_token = (
        env_data.get("BIBLIOTALK_API_KEY")
    ...
    env_data["BIBLIOTALK_API_KEY"] = api_token
    _write_env_file(env_path, env_data)
    return _apply_guru_env(base_dir)
