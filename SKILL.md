...
2. 从 **Gurutalk API** 拉取并同步人物 `profile.md`
...
1. 先确保上面的"首次初始化"已经完成。
2. 调用 `python scripts/gurutalk_client.py figures`
...
- `scripts/gurutalk_client.py` 会复制到对应的技能目录中
...
- 通过 `python scripts/gurutalk_client.py figure --slug {slug}` 拉取 `profile` 与 `profile_version`
...
## Gurutalk Client 命令参考

`scripts/gurutalk_client.py` 默认读取当前技能目录下的 `.env`。
...
- 每个技能目录都应保有自己的 `.env` 与 `scripts/gurutalk_client.py`
