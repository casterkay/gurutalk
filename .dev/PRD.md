...
> 版本说明版本对齐 `gurutalk/PRD.md`（后端 v1.2）。
>
> **核心架构**：**Gurutalk 负责数字人格 profile 的建立、维护、版本发布、检索与引用服务，但不生成回复、不调用 LLM**。同时明确引用短链 `q/{quote_id}` 为 **30 天临时引用**，并提供稳定公开分享页 `pub/{share_id}`（永久快照）。客户端/用户个人 Agent 负责调用 LLM 生成回答，并插入引用标记与短链。
>
...
**大师云 (GuruTalk)** 是一款 AI Agent 插件/客户端形态的交互入口。用户通过统一命令 `/{slug} {message}` 直接进入某位“大师”的对话界面；一旦会话开始，后续消息默认继续发送给当前人物，直到用户发送 `/gurutalk end`，或通过 `/{another-figure} {message}` 直接切换到另一位人物。客户端从云端 **Gurutalk API** 拉取该人物的数字人格 profile 与检索结果；最终回答由用户个人 Agent 自主调用 LLM 生成。
...
1. 用户发送 `/gurutalk`。
2. 客户端调用 Gurutalk `GET /v1/figures` 获取线上大师目录。
3. 用户输入 `/{slug} {message}` 进入对话。
4. 客户端调用 Gurutalk `GET /v1/figure/{slug}`。
5. Gurutalk 返回该人物的 profile。
6. 与本地 profile 版本（如存在）对照，如有更新，则将 profile 写入本地：
   - **Claude Code**: `~/.claude/skills/{slug}/profile.md`
   - **OpenClaw**: `~/.openclaw/workspace/skills/{slug}/profile.md`
7. 根据用户发送的消息 ，Agent 调用 Gurutalk `POST /v1/query`。
8. Gurutalk 在该人物的官方公有语料库中检索相关片段，返回结果列表与引用 ID。
9. 客户端将引用 ID 拼接为短链 `https://talks.guru/{quote_id}`。
10. 用户个人 Agent 基于本地 profile 与检索结果调用 LLM 生成最终回答，并在回答中插入引用标记与短链。
11. 用户点击短链时，打开 Gurutalk 的引用卡片页 `/{quote_id}`；程序也可调用 `GET /v1/quote/{quote_id}` 获取引用原始数据。
12. 用户可在 `/{quote_id}` 页面点击“分享”，生成稳定公开分享页 `/pub/{share_id}`（即使临时引用过期，分享页仍可访问）。
...
Gurutalk 需要为每个人物建立 `Persona Profile`。该 profile 的作用是帮助客户端和用户个人 Agent 更稳定地理解“这个人是谁、重视什么、如何表达、哪些材料应优先被视作其代表性观点”。

该 profile **用于约束检索理解与个人 Agent 的回答风格参考**，但 **不用于 Gurutalk 直接生成内容**。

### 4.2 Profile 结构

前五层由 Gurutalk 负责构建与维护，通过 API 同步到本地，第六层由用户个人 Agent 负责记录与更新。
...
API base URL：`https://api.talks.guru`

短链 base URL：`https://talks.guru/:quote_id`

公开分享 base URL：`https://talks.guru/pub/:share_id`

### 5.1 鉴权

- 使用 Better Auth。
- GuruTalk 初始化缺少 API key 时，应先获取用户 email，再由 agent 主动触发 magic link 发送；统一请求模板为：`GET https://talks.guru/login/magiclink?email={urlencoded_email}`
- `{urlencoded_email}` 必须是 URL 编码后的 email；agent 不应只让用户自己先打开登录页面。
- 用户点击邮件中的 magic link 后，在 `https://talks.guru/account/api-key` 获取或轮换自己的长期 `GURUTALK_API_KEY`，再复制回 agent。
- `https://talks.guru/login` 仍可作为人工登录入口，并支持 GitHub / Google / Apple OAuth。
- 所有运行时 Gurutalk API 请求均通过 `x-api-key: <GURUTALK_API_KEY>` 鉴权。
...
- `kind="chunk"`：可展示的原文片段，服务端会分配 `quote_id`，可用于 `/{quote_id}` 与后续分享。
...
- `quote_id` 对应临时引用短链：`https://talks.guru/{quote_id}`。
- 服务端会将临时引用以 `q:{quote_id}` 写入 Cloudflare KV，并设置 **30 天 TTL**；过期后 `/{quote_id}` 会返回“引用不存在”。
- 用户在 `/{quote_id}` 页面点击“分享”后，服务端会生成稳定公开分享链接：`https://talks.guru/pub/{share_id}`，并将 **完整 quote snapshot** 写入 `pub:{share_id}`（不设置过期），用于保证临时引用过期后依然可访问。
...
- 若用户点击 `https://talks.guru/{quote_id}` 返回“引用不存在”，客户端应提示该引用可能已过期（30 天 TTL）或无效。
...
4. 用户个人 Agent 可基于 profile 与检索结果生成回答，并插入 Gurutalk 短链。
5. 用户点击短链后，若引用未过期（30 天 TTL），可看到对应引用卡片页 `/{quote_id}`。
...
- 环境变量：`GURUTALK_API_KEY`
  - **Claude Code**: 配置在 `~/.claude/settings.json` 的 `env.GURUTALK_API_KEY`
  - **OpenClaw**: 配置在 `~/.openclaw/.env` 的 `GURUTALK_API_KEY`
...
- 从 Gurutalk API 拉取人物目录与 profile
...
- 自动解析对应的 skills 目录路径
- 从 Gurutalk API 拉取人物 profile 并生成 SKILL.md
...
5. 每条引用可点击查看 `https://talks.guru/{quote_id}`
